##############################################################################/
##############################################################################/
#Plotting distribution of a summary of fluidigm results
##############################################################################/
##############################################################################/
...
for (i in 3:(dim(logstattout)[2]-1)) {
  ...
  p + theme(axis.text.x = element_text(angle=90,size=12,hjust=0,vjust=0.5,
    ...
    ggtitle(colnames(logstattout)[i],
            subtitle="pval : ajouter ligne tableau") + #here for subtitle
    theme(plot.title = element_text(hjust = 0.5)) + 
    theme(plot.subtitle = element_text(hjust = 0.5)) + 
    ylim(-10,10)
  
  ggsave(file=paste("./output/pop-log(",colnames(logstattout)[i],").png",
...
pdf(file="./output/all-pop-log.pdf",width=11,height=8)
...
for (i in 3:(dim(logstattout)[2]-1)) {
  ...
  datadd$ordina<-seq.int(nrow(datadd))
  print(
    ggplot(data=datadd,aes(x=reorder(sorteddatatoutpheno,ordina),
                           y=get(colnames(logstattout)[i]),
                           fill=sorteddatatoutpheno_1),
           position=position_dodge()) + 
      geom_bar(stat="identity") +
      ...
      theme(axis.text.x = element_text(angle=90,size=12,hjust=0,vjust=0.5,
                                       colour=colopop[datadd$popula])) + 
      scale_fill_manual(values=c("#FF0000","#33CC33")) + 
      theme(legend.position="bottom") + 
      xlab("individus") + ylab("log2(ddct)") + 
...
##############################################################################/
#one pdf file, ordered by ascending log2 
##############################################################################/

pdf(file="./output/all-log.pdf",width=11,height=8)
